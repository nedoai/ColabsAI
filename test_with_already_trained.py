...
model = joblib.load('character_model.pkl')
...
while True:
    ...
    for idx in uncertain_feature_indices:
        ...
        for character_idx in range(len(characters_data)):
            X_copy = np.copy(X)
    ...
    answer = input(f"Ваш персонаж {uncertain_feature_name}? (да/нет/возможно да/возможно нет/между): ").lower()

    X_copy = np.copy(X)
    if answer == 'да':
        for i in range(len(X_copy)):
            X_copy[i][uncertain_feature_index] = 10
        answered_features.add(uncertain_feature_index)
    elif answer == 'нет':
        for i in range(len(X_copy)):
            X_copy[i][uncertain_feature_index] = 0
        answered_features.add(uncertain_feature_index)
    elif answer == 'возможно да':
        for i in range(len(X_copy)):
            X_copy[i][uncertain_feature_index] = 6
        answered_features.add(uncertain_feature_index)
    elif answer == 'возможно нет':
        for i in range(len(X_copy)):
            X_copy[i][uncertain_feature_index] = 3
        answered_features.add(uncertain_feature_index)
    elif answer == "между":
        for i in range(len(X_copy)):
            X_copy[i][uncertain_feature_index] = 5
        answered_features.add(uncertain_feature_index)
    else:
